...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  if (parentID == 0) 
  {
    ...
    if (fEventAction && fEventAction->GetInteractionType() == -1) 
    {
        ...
        if (volume && volume == fDetConstruction->GetAbsorberPV()) {  // Interaction happens inside calorimeter
          // G4cout << "Position Z: " << position.z() << G4endl;
          fEventAction->SetInteractionDepth(position.z() + fDetConstruction->GetCalorThickness() / 2 );  // Depth in detector
          // G4cout << "Depth Z: " << position.z() + fDetConstruction->GetCalorThickness() / 2  << G4endl;
          fEventAction->SetInteractionLayer(step->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(1)) ;  // Layer number, while GetCopyNumber(depth=1) find its mother volume
          // G4cout << "Layer: " << step->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(1) << G4endl;
