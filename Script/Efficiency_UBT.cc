void Efficiency_UBT()
{
    ...
    double Helium3_Eff[18]={0};
    double Helium3_Eff_UBT[18]={0};

    double Carbon_Eff[18]={0};
    double Carbon_Eff_UBT[18]={0};
    ...
    TH1D *h1_H[18]; TH1D *h2_H[18]; TH1D *h3_H[18];
    TH1D *h1_c[18]; TH1D *h2_c[18]; TH1D *h3_c[18];
    ...
    for (int i = 0; i < 19; i++)
    {
        ...
        Proton_Eff[i]     = h2_p[i]->Integral()/h1_p[i]->Integral();
        ...
        Deuteron_Eff[i]     = h2_d[i]->Integral()/h1_d[i]->Integral();
        ...
        Electron_Eff[i]     = h2_e[i]->Integral()/h1_e[i]->Integral();
        ...
        Helium4_Eff[i]     = h2_h[i]->Integral()/h1_h[i]->Integral();
        ...
        auto helium3_tree = (TTree*)helium4_file->Get("B4");
        h1_H[i] = new TH1D(Form("h1_H[%d]",i),Form("h1_H[%d]",i),100,0,1);
        h2_H[i] = new TH1D(Form("h2_H[%d]",i),Form("h2_H[%d]",i),100,0,1);
        h3_H[i] = new TH1D(Form("h3_H[%d]",i),Form("h3_H[%d]",i),100,0,1);
        ...
        Helium3_Eff[i]     = h2_H[i]->Integral()/h1_H[i]->Integral();
        ...
        auto carbon_tree = (TTree*)helium4_file->Get("B4");
        ...
        Carbon_Eff[i]     = h2_c[i]->Integral()/h1_c[i]->Integral();
    ...
    auto gre_H     = new TGraphErrors(18, Energy, Helium3_Eff, Energy_Err, Uncertainty);
    auto gre_H_UBT = new TGraphErrors(18, Energy, Helium3_Eff_UBT, Energy_Err, Uncertainty);
    ...
    auto gre_c     = new TGraphErrors(18, Energy, Carbon_Eff, Energy_Err, Uncertainty);
    auto gre_c_UBT = new TGraphErrors(18, Energy, Carbon_Eff_UBT, Energy_Err, Uncertainty);
