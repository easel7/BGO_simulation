    ...
    for (int i = 0; i < 13; i++) // Deposit Energy Bin
    {
        ...
        for (int j= 0; j<15 ; j++) // Layer
        {
            ...
            if(j<=13) 
            {
                ...
                deuteron_tree->Draw(Form("log10(L%d_E)>>h1_d[%d][%d]",j,i,j),HET,""); h1_d[i][j]->Sumw2(); h1_d[i][j]->Scale(1.0/h1_d[i][j]->Integral());h1_d[i][j]->SetLineColor(kBlue);    h1_d[i][j]->SetMarkerColor(kBlue);    h1_d[i][j]->SetLineWidth(2);
                electron_tree->Draw(Form("log10(L%d_E)>>h1_e[%d][%d]",j,i,j),HET,""); h1_e[i][j]->Sumw2(); h1_e[i][j]->Scale(1.0/h1_e[i][j]->Integral());h1_e[i][j]->SetLineColor(kOrange-3);h1_e[i][j]->SetMarkerColor(kOrange-3);h1_e[i][j]->SetLineWidth(2);
                helium4_tree ->Draw(Form("log10(L%d_E)>>h1_h[%d][%d]",j,i,j),HET,""); h1_h[i][j]->Sumw2(); h1_h[i][j]->Scale(1.0/h1_h[i][j]->Integral());h1_h[i][j]->SetLineColor(kGreen-3); h1_h[i][j]->SetMarkerColor(kGreen-3); h1_h[i][j]->SetLineWidth(2);
                helium3_tree ->Draw(Form("log10(L%d_E)>>h1_H[%d][%d]",j,i,j),HET,""); h1_H[i][j]->Sumw2(); h1_H[i][j]->Scale(1.0/h1_H[i][j]->Integral());h1_H[i][j]->SetLineColor(kGreen-3); h1_H[i][j]->SetMarkerColor(kGreen-3); h1_H[i][j]->SetLineWidth(2);
