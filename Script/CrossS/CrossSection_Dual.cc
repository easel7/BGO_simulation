void CrossSection_Dual()
{
    ...
    auto h1 = new TH1D("h1","h1",40,0,400);
    tree1->Draw("First_Had_Depth>>h1",HI,"");
    h1->SetTitle("100 GeV Inelastic Hadronic Depth Distribution;Depth(mm);Counts");

    auto file2 = TFile::Open("/Users/xiongzheng/software/B4/B4c/Root/Deuteron_100GeV.root");
    auto tree2 = (TTree*)file2->Get("B4");
    auto h2 = new TH1D("h2","h2",40,0,400);
    tree2->Draw("First_Had_Depth>>h2",HI,"");
    h2->SetTitle("100 GeV Inelastic Hadronic Depth Distribution;Depth(mm);Counts");

    TF1 *fitFunc1 = new TF1("fitFunc1", "[0]*exp(-x/[1])", 50, 250); fitFunc1->SetParameters(100, 10); fitFunc1->SetLineColor(kRed);
    TF1 *fitFunc2 = new TF1("fitFunc2", "[0]*exp(-x/[1])", 50, 250); fitFunc2->SetParameters(100, 10); fitFunc2->SetLineColor(kBlue);
    ...
    latex.DrawLatex(5,70,"Fitting Function: P=Const. *exp(-x/#lambda)");
    latex.DrawLatex(5,55,Form("Proton Fitting #lambda: %.2f mm",lambda1));
    latex.DrawLatex(5,40,Form("Deuteron Fitting #lambda: %.2f mm",lambda2));
